...
{ // Fade in all outputs
  ...
  for (int k = 0; k < (maxBrightness * ((double)percent / 100) * (numOutputs)) + delayForCompletion; k++)
  {
    delay(5); // Use delayMicroseconds(us) for faster fade than delay(1) which is in milliseconds. 
    ...
    //Serial.print(k);Serial.print(" ");Serial.println(ledCount);
  }
  ledCount = 1;
  prevBri = 0;
  printStatus();
  ...
  for (int k = 0; k < (maxBrightness * ((double)percent / 100) * (numOutputs)) + delayForCompletion; k++)
  {
    ...
    for (int i = 0; i < ledCount; i++)
    {
      if ( i >= listSize )
      {
        if (!isEndSet)
        {
          k = (maxBrightness * ((double)percent / 100) * (numOutputs));
          isEndSet = true;
        }
        else
        {
